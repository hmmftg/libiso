...
func (f *Field) ValueFromString(data string) ([]byte, error) {
	...
	if f.HasChildren() {
		...
		for _, child := range f.Children {
			switch child.Type {
			case FixedType:
				subData := data[offset:child.Size]
				...
				offset += child.Size
			case VariableType:
				subDataLen, _ := strconv.Atoi(data[offset:child.LengthIndicatorSize])
				subData := data[offset+child.LengthIndicatorSize : subDataLen]
				subByte, err := child.ValueFromString(subData)
				if err != nil {
					return nil, err
				}
	...
	case IRANSYSTEM:
		return iransystem.Decode(data), nil
	case EBCDIC:
